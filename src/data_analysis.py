...
def missing(df):
    ...
    missing_info = pd.DataFrame({'Missing Values': missing_values,'Missing Rate (%)': missing_rate})
...
def interactive_correlation_heatmap(df):
    ...
    fig = px.imshow(corr_matrix,text_auto=True, color_continuous_scale='Blues', title="Interactive Correlation Heatmap")
    fig.update_layout(xaxis_title='Features',yaxis_title='Features',title_font_size=20,)
    fig.show()
